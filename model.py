    ...
    if fit:
        ...
        for col in num_cols:
            if col in data.columns:
                ...
                print(
                    f"📌 {col} → Outliers found: {len(outliers)} (Lower: {lb:.2f}, Upper: {ub:.2f})"
                )
...
def predict_model(
    ...
    data_processed, _, _ = preprocess_data(
        data, scaler=scaler, label_encoders=label_encoders, fit=False
    )
...
if __name__ == "__main__":
    ...
    for customer in customers:
        ...
        print(
            f"✅ Predicted Loan Approval: {'Approved' if predicted_approval[0] == 1 else 'Rejected'}"
        )
